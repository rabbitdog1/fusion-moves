...
本文件是在 ppikktetm_s3.py 基础上的"小幅修改"优化版：
  1. 算法核心逻辑完全保留（ETM-ASM、Fusion Moves、QPBO、S3 等）。
...
def asm_fusion_moves_optimization(cfg, initial_solution: np.ndarray,
    ...
    ==== MODIFIED: 内存优化 + 重复计算消除 ====
      ...
      2. 删除每次迭代里"redundant" 的三次 S3 诊断重复计算
         （current/candidate/fused 已经分别算过了）。
         原代码在每个候选解内重复调用 S3() 三次 + asm_evaluate_solution 一次。
    ...
    current_objective = S3(cfg.A1, cfg.A2, current_solution)
    ...
    for iteration in range(max_iterations):
        ...
        for cand_idx in range(num_candidates):
            ...
            candidate_objective = S3(cfg.A1, cfg.A2, candidate_solution)
    ...
    initial_objective_for_info = S3(cfg.A1, cfg.A2, initial_solution)
    result_info = {
...
if __name__ == '__main__':
    ...
    results_df.to_excel("our+sana_s3_t10.xlsx", index=False)
    print("\n结果已保存至 our+sana_s3_t10.xlsx")
